...
```
        ...
        self.driver = webdriver.PhantomJS(executable_path="/usr/local/bin/phantomjs",service_args=['--ignore-ssl-errors=yes',
    ...
    def test_geo_config(self):
        ...
        #errors.append("server " + self.server)
        #errors.append("cert " + self.cert)
        #errors.append("certkey " + self.certkey)
        #errors.append("certpass " + self.certpass)
        #errors.append("server_args " + str(self.server_args))

        if not os.path.exists(self.creates):
            try:
#                driver = self.driver
#                #print self.base_url + "/example_module/web/"
#                driver.get(self.base_url + "/example_module/web/")
#                #driver.get_screenshot_as_file("/tmp/screen.png")
#                #errors.append("source " + driver.page_source)
#                
...
#                driver.find_element_by_link_text("Add layers").click()
#                Select(driver.find_element_by_id("id5")).select_by_visible_text("tcri:tcri_so")
#                time.sleep(10)
#                #print driver.page_source
#                driver.find_element_by_id("id14").click()
#                #driver.find_element_by_css_selector("//span/a/span").click()
...
def main():
    #example_module = ExampleModule("172.26.14.35:8443", "./c3test_u_fouo.crt.pem", "./c3test_u_fouo.key.pem", "c3password1", "/tmp/geosotest")
    #def __init__(self, server, cert, certkey, certpass, creates):
